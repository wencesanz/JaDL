...
function setMeta(route) {
  ...
  if (route.view === "studio") {
    ...
    if (s) {
      ...
      jsonld = [
        studioLd(s, url, desc),
        breadcrumbLd([["Studios", SITE_BASE + "/studios"], [s.name, url]]),
      ];
    }
...
function App() {
  ...
  useEffect(() => {
    // Don't touch the URL (or SEO meta) until studios.json has loaded — otherwise
    // the initial "index" placeholder route (used while data is still loading)
    // overwrites a real deep link like /studio/oddity-studio with "/" before
    // it ever gets the chance to resolve correctly.
    if (!ready) return;
